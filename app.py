...
for col,ed,w,hc,bg,pin in cols_cfg:
    opts={'editable':ed,'width':w,'headerClass':hc,'cellStyle':{'backgroundColor':bg}}
...
resp = AgGrid(df, gridOptions=grid_opts, data_return_mode=DataReturnMode.FILTERED_AND_SORTED,
...
# baseline G
g0 = st.number_input("Baseline G (Day1)", min_value=0.0, step=0.1, format="%.2f")
...
df2['চাল প্রাপ্তি (F)'] = df2['গ্রহণের পরিমাণ (D)'] * RATE
    G = [g0 := g0 if False else g0]  # placeholder
    G = [g0]
    for i in range(len(df2)):
        if i == 0:
            ...
            # new logic: today's E increases available after subtracting F
            G.append(prev - F_i + E_i)
    df2['G (চাল ব্যবহার)'] = G
    ...
    AgGrid(df2, gridOptions=gb2.build(), fit_columns_on_grid_load=True,
           height=400, theme=theme)
